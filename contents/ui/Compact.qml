...
Item {
    ...
    readonly property bool showSPlayerInCompact: plasmoid.configuration.showSPlayerLyricsInCompact

    Layout.preferredWidth: {
        if (widget.splayerOnline && showSPlayerInCompact) {
            return splayerContent.implicitWidth + lengthMargin * 2
        }
        return horizontal ? grid.implicitWidth + lengthMargin * 2 : grid.implicitWidth
    }
    Layout.preferredHeight: {
        if (widget.splayerOnline && showSPlayerInCompact) {
            return splayerContent.implicitHeight + lengthMargin * 2
        }
        return !horizontal ? grid.implicitHeight + lengthMargin * 2 : grid.implicitHeight
    }
    ...
    Loader {
        id: splayerContent
        anchors.fill: parent
        active: widget.splayerOnline && showSPlayerInCompact
        visible: active
        sourceComponent: SPlayerCompactContent {}
    }
